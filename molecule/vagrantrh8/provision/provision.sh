#!/bin/bash
# shellcheck disable=SC2004,SC2006,SC2034,SC2181,SC2086

#CAPSULE TO REGISTER THE HOST TO
DEFAULT_CAPSULE="capsulersipre.test.es"
...
hostnamectl set-hostname "${HOSTNAME}.test.es"
...
bash /root/postinstall/satellite62.sh --action register --capsule ${DEFAULT_CAPSULE} --content ${DEFAULT_CHANNEL} --host ${HOSTNAME}.test.es &>/dev/null || exit 1
